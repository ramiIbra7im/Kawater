...
function Posts() {
    ...
    return (
        <div className="container">
            ...
            {loading ? (
                <motion.div className="text-center fs-4 fw-bold p-2 text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                    ⏳... جاري تحميل الخواطر
                </motion.div>
            ) : (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                    {filteredPosts.map((post, index) => (
                        <motion.div key={index} className="post p-3 col-lg-8 m-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                            ...
                            <div className="content fs-5 pt-1 pb-3 text-end" id="content">{post.text}</div>

                            <div className="d-flex justify-content-between align-items-center ">
                            <div className=" text-end fs-6 date"> تاريخ النشر: {post.date}</div>
